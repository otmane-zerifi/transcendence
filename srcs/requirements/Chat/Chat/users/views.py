...
from .utils import generate_otp, save_image
...
class UserProfile(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({"message": "User Profile is Retrivied Successfuly!", "user": userData}, status=status.HTTP_200_OK)
    ...
    def put(self, request):
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                return Response({"message": "User Profile is Successfuly Updated", "user": userData}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class UserPublicProfile(APIView):
    permissions = [IsAuthenticated, CheckBlacklistedRefreshTokenPermission, Check2FAPermission]
